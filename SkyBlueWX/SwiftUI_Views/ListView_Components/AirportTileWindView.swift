...
struct AirportTileWindView: View {
    ...
    var windInfoString : String {
        ...
        var stringBuilder : String = "\(windDirectionString) @ \(report.windSpeedToString(unit: cockpit.settings.speedUnit))"
        if report.wind.hasGusts {
            stringBuilder += " G \(report.windGustsToString(unit: cockpit.settings.speedUnit))\(cockpit.settings.speedUnitString)"
        } else {
