    ...
    var body: some View {
        if let validEntry = entry {
            HStack {
                ...
                Image(systemName: (cockpit.queryCodes.contains(validEntry.icao) ? "minus.square" : "plus.square")).foregroundColor((cockpit.queryCodes.contains(validEntry.icao) ? Color.darkRed : Color.darkGreen)).frame(width: UIScreen.main.bounds.width * 0.1).onTapGesture {
