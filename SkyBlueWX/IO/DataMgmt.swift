...
struct DataBaseHandler {
    func openConnection() throws -> OpaquePointer? {
        ...
        guard let dbPath = Bundle.main.url(forResource: "airdata", withExtension: "db") else {
            throw DataBaseHandlerError.missingDataBaseFile
        }
    ...
    private func runQuery(query: String, returnType: QueryReturnType?) -> AirportDict? {
        ...
        func parseAirportRow() -> Airport {
            ...
            let ident = sqlite3_column_text(statement, 12) != nil ?
            String(cString: sqlite3_column_text(statement, 12)!): ""
            let name = sqlite3_column_text(statement, 3) != nil ?
        ...
        if sqlite3_prepare_v2(pointer, query, -1, &statement, nil) == SQLITE_OK {
            ...
            guard returnType != nil else {
                if sqlite3_step(statement) == SQLITE_DONE {closeAsPass()} else {closeAsFail()}
                return nil
            }
